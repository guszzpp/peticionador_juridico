...
def processar_arquivo(pdf):
    ...
    try:
        ...
        resultado = processar_peticao(
            primeira_pagina, texto, CAMINHO_MODELOS, MODELO_PADRAO
        )
...
with gr.Blocks(css=Path(CAMINHO_CSS).read_text()) as demo:
    ...
    with gr.Row():
        arquivo_pdf = gr.File(
            label="Upload do Arquivo", type="filepath", file_types=[".pdf"]
        )
        modelos_reutilizaveis = gr.Textbox(
            label="Modelos Reutilizáveis", interactive=False
        )
